 ...
 */
@Suppress("UNCHECKED_CAST")
infix fun <S, T: Any> TableColumn<S, T>.value(cellValueFactory: (TableColumn.CellDataFeatures<S, T>) -> Any?) = apply {
    this.cellValueFactory = Callback<TableColumn.CellDataFeatures<S, T>, ObservableValue<T>> {
        ...
        (createdValue as? ObservableValue<T>) ?: SimpleObjectProperty<T>(createdValue as? T)
    }
}
