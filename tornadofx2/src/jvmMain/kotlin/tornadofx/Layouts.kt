...
inline fun <reified R> Node.builderTarget(): KFunction1<R, ObjectProperty<Node>>? =
  properties["tornadofx.builderTarget"] as? KFunction1<R, ObjectProperty<Node>>?

fun <R> Node.builderTarget(value: KFunction1<R, ObjectProperty<Node>>?) {
  properties["tornadofx.builderTarget"] = value
}

fun Node.resetBuilderTarget() {
  properties["tornadofx.builderTarget"] = null
}
...
internal fun BorderPane.region(region: KFunction1<BorderPane, ObjectProperty<Node>>?, op: BorderPane.() -> Unit) {
  builderTarget<BorderPane>(region)
  op()
  resetBuilderTarget()
}
