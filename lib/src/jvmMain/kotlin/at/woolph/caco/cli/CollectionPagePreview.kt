...
class CollectionPagePreview(
  ...
  suspend fun printLabel(setCode: String, file: Path) = coroutineScope {
    val cardList = transaction {
      (ScryfallCardSet.findByCode(setCode)?.cardPrints ?: emptyList()).sortedBy {
        it.collectorNumber
      }
    }
    ...
    val cardListWithImages =
        cardList.map { card ->
          card to
              ImageCache.getImageByteArray(card.thumbnail.toString()) {
                    Either.catch {
                      progressTracker.updateContext(
                          "card #\$${card.collectorNumber} ${card.name} image downloading\r"
                      )
      ...
      collectionPages.forEachIndexed { pageNumber, pageContent ->
        page(pageFormat) {
          pageContent.forEachIndexed { index, it ->
            ...
            try {
              progressTracker.updateContext(
                  "card #\$${card.collectorNumber} ${card.name} image rendering\r"
              )
