        ...
        frame(margin, margin, margin, margin) {
          ...
          frame(0f, setDivYPos, 0f, 0f) {
            ...
            set.subIconRight?.toPDImage(this)?.let {
              drawAsImageCentered(it, maximumWidthSub, desiredHeightSub, xOffsetIcons, yOffsetIcons)
            }
            ...
            set.mainIcon?.toPDImage(this)?.let {
              drawAsImageCentered(it, maximumWidth, desiredHeight, 0f, mainIconYPos)
            }
          }
        }
      }
...
fun main() {
  ...
  transaction {
    val thresholdTooMuchPages =
        45 // TODE fine tune the max card count per page, so that the labels are not too small
    val thresholdTooFewPages = 7

    val labels =
        sequence<MapLabelItem> {
