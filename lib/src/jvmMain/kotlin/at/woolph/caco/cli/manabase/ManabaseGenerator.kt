          ...
          .map { manaColor ->
            manaColor to
                mapNotNull { it.card.manaCost }
                    .sumOf { manaCost ->
                      Math.pow(
                              Regex(Regex.escape(manaColor.symbol))
                                  .findAll(manaCost)
                                  .count()
                                  .toDouble(),
                              2.0,
...
fun Collection<DecklistEntryCard>.meanPipRequirement() =
    PipDistribution(
        ManaColor.entries
            .associateWith { manaColor ->
              this@meanPipRequirement.filterNot { it.card.isLand }
                  .mapNotNull { it.card.manaCost }
                  .map { Regex(Regex.escape(manaColor.symbol)).findAll(it).count() }
                  .average()
            }
            .filterValues { it > 0.0 }
    )

fun Collection<Land>.meanProduction(totalCount: Int = this@meanProduction.size) =
    ManaColor.entries.associateWith { color ->
      this@meanProduction.count { color in it.manaProduction }.toDouble() / totalCount
    }
...
fun generateManabase(
  ...
  // each basic once
  filteredBasicLands
      .filter { neededColors.contains(it.manaProduction.single()) }
      .forEach { pickedLand ->
        selectedLands.add(pickedLand)
        println("picked ${pickedLand.name}")
      }
  ...
  println(
      "productionDistribution = ${ManaColor.entries.associateWith { color -> lands.filter { (land, _) -> color in land.manaProduction }.sumOf { (_, count) -> count }.toDouble() / lands.size }}"
  )
...
class ColorIdentity(val colorIdentity: Set<ManaColor>) {
  constructor(
      vararg color: ManaColor
  ) : this(
      if (color.isEmpty()) EnumSet.noneOf(ManaColor::class.java) else EnumSet.copyOf(color.asList())
  )
  ...
  operator fun plus(colorIdentity: Set<ManaColor>) =
...
open class Land(
  ...
  fun desirability(
      ...
      alreadyProduced: Map<ManaColor, Double>,
  ) =
      multiply(
          desirableFactor,
  ...
  fun multiply(vararg factors: Double) = factors.reduce { acc, d -> acc * d }
}
...
val snowBasicLands =
    listOf(
            Land(
                "Snow-Covered Plains",
                ColorIdentity.WHITE,
                basicTypes = ColorIdentity.WHITE,
                basic = true,
                snow = true,
            ),
            Land(
            ...
            Land(
                "Snow-Covered Swamp",
                ColorIdentity.BLACK,
                basicTypes = ColorIdentity.BLACK,
                basic = true,
                snow = true,
            ),
            Land(
            ...
            Land(
                "Snow-Covered Forest",
                ColorIdentity.GREEN,
                basicTypes = ColorIdentity.GREEN,
                basic = true,
                snow = true,
            ),
    ...
    listOf(
        ...
        object :
            Land(
                ...
                ColorIdentity.FIVE_COLORED.colorIdentity,
            ) {
        ...
        Land(
            "Indatha Triome",
            ColorIdentity.ABZAN,
            basicTypes = ColorIdentity.ABZAN,
            tapLand = true,
        ),
        Land(
            "Ketria Triome",
            ColorIdentity.TEMUR,
            basicTypes = ColorIdentity.TEMUR,
            tapLand = true,
        ),
        Land(
            "Raugrin Triome",
            ColorIdentity.JESKAI,
            basicTypes = ColorIdentity.JESKAI,
            tapLand = true,
        ),
        Land(
            "Zagoth Triome",
            ColorIdentity.SULTAI,
            basicTypes = ColorIdentity.SULTAI,
            tapLand = true,
        ),
        Land(
            "Spara's Headquarters",
            ColorIdentity.BANT,
            basicTypes = ColorIdentity.BANT,
            tapLand = true,
        ),
        Land(
            "Raffine's Tower",
            ColorIdentity.ESPER,
            basicTypes = ColorIdentity.ESPER,
            tapLand = true,
        ),
        Land(
            "Xander's Lounge",
            ColorIdentity.GRIXIS,
            basicTypes = ColorIdentity.GRIXIS,
            tapLand = true,
        ),
        Land(
            "Ziatora's Proving Ground",
            ColorIdentity.JUND,
            basicTypes = ColorIdentity.JUND,
            tapLand = true,
        ),
        Land(
            "Jetmir's Garden",
            ColorIdentity.NAYA,
            basicTypes = ColorIdentity.NAYA,
            tapLand = true,
        ),
        ...
        Land(
            "Hallowed Fountain",
            ColorIdentity.AZORIOUS,
            basicTypes = ColorIdentity.AZORIOUS,
            pain = true,
        ),
        ...
        Land(
            "Temple Garden",
            ColorIdentity.SELESNYA,
            basicTypes = ColorIdentity.SELESNYA,
            pain = true,
        ),
        Land(
            "Godless Shrine",
            ColorIdentity.ORZHOV,
            basicTypes = ColorIdentity.ORZHOV,
            pain = true,
        ),
        Land("Steam Vents", ColorIdentity.IZZET, basicTypes = ColorIdentity.IZZET, pain = true),
        Land(
            "Overgrown Tomb",
            ColorIdentity.GOLGARI,
            basicTypes = ColorIdentity.GOLGARI,
            pain = true,
        ),
        ...
        object :
            Land(
                "Grand Coliseum",
                ColorIdentity.COLORLESS,
                ColorIdentity.FIVE_COLORED.colorIdentity + ManaColor.Colorless,
                tapLand = true,
                pain = true,
            ) {
          override fun additionalDesirability(selectionCriterion: SelectionCriterion) =
        ...
        Land(
            "Adarkar Wastes",
            ColorIdentity.AZORIOUS,
            ColorIdentity.AZORIOUS.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Underground River",
            ColorIdentity.DIMIR,
            ColorIdentity.DIMIR.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Sulfurous Springs",
            ColorIdentity.RAKDOS,
            ColorIdentity.RAKDOS.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Karplusan Forest",
            ColorIdentity.GRUUL,
            ColorIdentity.GRUUL.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Brushland",
            ColorIdentity.SELESNYA,
            ColorIdentity.SELESNYA.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Caves of Koilos",
            ColorIdentity.ORZHOV,
            ColorIdentity.ORZHOV.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Shivan Reef",
            ColorIdentity.IZZET,
            ColorIdentity.IZZET.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Llanowar Wastes",
            ColorIdentity.GOLGARI,
            ColorIdentity.GOLGARI.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Battlefield Forge",
            ColorIdentity.BOROS,
            ColorIdentity.BOROS.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        Land(
            "Yavimaya Coast",
            ColorIdentity.SIMIC,
            ColorIdentity.SIMIC.colorIdentity + ManaColor.Colorless,
            pain = true,
        ),
        // Utility Colorless
        ...
        Land(
            "Scene of the Crime",
            ColorIdentity.COLORLESS,
            manaProduction = ColorIdentity.FIVE_COLORED.colorIdentity + ManaColor.Colorless,
            desirableFactor = 0.8,
            tapLand = true,
            artifact = true,
        ),
        ...
        Land(
            "Undercity Sewers",
            ColorIdentity.DIMIR,
            basicTypes = ColorIdentity.DIMIR,
            tapLand = true,
            surveil = true,
            desirableFactor = 1.2,
        ),
        Land(
            "Raucous Theater",
            ColorIdentity.RAKDOS,
            basicTypes = ColorIdentity.RAKDOS,
            tapLand = true,
            surveil = true,
            desirableFactor = 1.2,
        ),
        Land(
            "Commercial District",
            ColorIdentity.GRUUL,
            basicTypes = ColorIdentity.GRUUL,
            tapLand = true,
            surveil = true,
            desirableFactor = 1.2,
        ),
        ...
        Land(
            "Thundering Falls",
            ColorIdentity.IZZET,
            basicTypes = ColorIdentity.IZZET,
            tapLand = true,
            surveil = true,
            desirableFactor = 1.2,
        ),
        ...
        Land(
            "Elegant Parlor",
            ColorIdentity.BOROS,
            basicTypes = ColorIdentity.BOROS,
            tapLand = true,
            surveil = true,
            desirableFactor = 1.2,
        ),
        Land(
            "Hedge Maze",
            ColorIdentity.SIMIC,
            basicTypes = ColorIdentity.SIMIC,
            tapLand = true,
            surveil = true,
            desirableFactor = 1.2,
        ),
...
class FetchLand(
  ...
  override fun additionalDesirability(selectionCriterion: SelectionCriterion) =
      fetchingIdentity.colorIdentity.count {
        it in selectionCriterion.commanderColorIdentity.colorIdentity
      } * 0.5
}
...
class PipDistribution(val pipDistribution: Map<ManaColor, Double>) {
  ...
  fun weighting(land: Land) =
      pipDistribution.entries
          .filter { (color, _) -> land.manaProduction.contains(color) }
          .sumOf { (_, pipWeight) -> pipWeight }

  operator fun minus(meanProduction: Map<ManaColor, Double>) =
      PipDistribution(
