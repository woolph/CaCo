...
fun main() {
  ...
  getLands()
      ...
      .forEach { (landCard, _, doesTapForManaImmediately) ->
        ...
        println(
            "${landCard.name} - $colors - $production - doesTapForManaImmediately: $doesTapForManaImmediately - ${landCard.lowestPrice}"
        )
      }
}
  ...
  Card.find { Cards.type like "%Land%" }
      ...
      .map { landCard ->
        ...
        LandCard(
            ...
            doesTapForManaImmediately =
                !(entersTapped || landCard.type?.contains("Creature") == true),
        )
      }
}
