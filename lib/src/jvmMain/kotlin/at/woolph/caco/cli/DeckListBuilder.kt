...
class DeckListBuilder {
  ...
  fun add(deckZone: DeckZone, name: String, amount: Int) = apply {
    deckZones
        .computeIfAbsent(deckZone) { mutableMapOf() }
  ...
  fun validate() {
    DeckZone.entries.forEach { deckZone ->
      if (deckZone.isMandatory)
          require(!format.zones.contains(deckZone) || !deckZones[deckZone].isNullOrEmpty()) {
            "$deckZone must not be empty for format $format"
          }
      else
          require(format.zones.contains(deckZone) || deckZones[deckZone].isNullOrEmpty()) {
            "$deckZone should be empty for format $format"
          }
    }
  }
  ...
  fun build(): DeckList {
    ...
    return DeckList(
        name =
            name
                ?: if (format.zones.contains(DeckZone.COMMAND_ZONE))
                    deckZones[DeckZone.COMMAND_ZONE]!!.keys.joinToString(" & ") { it }
                else "<unnamed>",
        format = format,
