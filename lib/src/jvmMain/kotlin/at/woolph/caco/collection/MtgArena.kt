...
val MTGA_LOG_FILE =
    File(System.getenv("APPDATA") + "\\..\\LocalLow\\Wizards Of The Coast\\MTGA\\output_log.txt")
...
fun importArenaCollection() {
  ...
  MTGA_LOG_FILE.forEachLine { line ->
    if (copy) {
      regex.find(line)?.let { result.put(it.groupValues[1].toInt(), it.groupValues[2].toInt()) }
    }
  ...
  transaction {
    result.forEach { arenaId, importedCount ->
      ...
      if (importedCard != null) {
        ArenaCardPossession.find { ArenaCardPossessions.card.eq(importedCard.id) }
            .singleOrNull()
            ?.let { it.count = importedCount }
            ?: ArenaCardPossession.new {
              card = importedCard
              count = importedCount
            }
      } else {
        // throw IllegalStateException("database does not know arenaId = $arenaId or there are
        // multiples => import from scryfall? https://api.scryfall.com/cards/arena/$arenaId")
        println(
            "unable to import $arenaId because it's not known https://api.scryfall.com/cards/arena/$arenaId"
        )
      }
    }
  }
