...
fun importSequenceDeckbox(
  ...
  return importSequence(file) { nextLine -> mapDeckbox(nextLine, knownSets) }
}
...
fun Raise<Throwable>.mapDeckbox(
  ...
  val (setCode, setName, isPromo, token, isTheListCard) =
      editionToSetName(
          ...
          isTheListCardStoredAsRegularVersionWithArtistProofTag =
  ...
  val purchasePrice = nextLine["My Price"]?.removePrefix("$")?.toDoubleOrNull()?.takeIf { it > 0.0 }
  ...
  val cardName =
      nextLine["Name"]!!
          ...
          .let {
            when (it) {
              in listOf("Plains", "Island", "Swamp", "Mountain", "Forest", "Command Tower") if
                  setCode == "rex"
               -> "$it // $it"
              ...
              "Checklist" -> {
                when (setCode) {
                  "tsoi" ->
                      when (val number = nextLine["Card Number"]!!) {
                        "19" -> "Shadows Over Innistrad Checklist 1"
                        "20" -> "Shadows Over Innistrad Checklist 2"
                        else ->
                            throw NoSuchElementException("unknown checklist card number $number")
                      }
          ...
          .let {
            if (it.startsWith("Emblem: ")) "${it.removePrefix("Emblem: ")} Emblem"
            else {
              it
            }
          }
  val cardNumber =
      mapSetNumber(nextLine["Card Number"]!!, setName)
          .let {
            when (cardName) {
              "Innistrad Checklist",
              ...
              "Thallid" if it == "87" ->
                  "74a" // FIXME find a more elegant solution to this issue (e.g. for sets with this
              // kind of set numbering, generate a deckbox numbering
              ...
              "Lat-Nam's Legacy" if it == "45" -> "30b"
                ...
                if (printingNote matches Regex("[a-z]")) "$it$printingNote"
                else if (setName == "War of the Spark Japanese Alternate Art") "$it★ P" else it
              }
            }
          }
          .let {
  ...
  if (token && cardName.contains(" // ")) {
    ...
    raise(
        IntentionallySkippedException(
            "skipping $cardName cause Ability Punchcard's are not covered by scryfall"
        )
    )
  } else {
    val card =
    ...
    val finish =
        when {
          stampPrereleaseDate || nextLine["Foil"] == "foil" ->
              when {
                !card.finishes.contains(Finish.Foil) && card.finishes.contains(Finish.Etched) ->
                    Finish.Etched
                else -> Finish.Foil
              }
          else -> Finish.Normal
    ...
    return CardCollectionItem(
        ...
        cardCollectionItemId =
            CardCollectionItemId(
                ...
                variantType =
                    when {
                      stampPlaneswalkerSymbol && stampPrereleaseDate ->
                          raise(
...
internal val DATE_FORMAT_DECKBOX =
    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss z").withZone(ZoneId.from(ZoneOffset.UTC))
...
fun determineScryfallSetName(
    edition: String,
    isPromo: Boolean,
    isToken: Boolean,
    isSubstituteCard: Boolean,
) =
    mapSetName(
            allSuffixes.fold(allPrefixes.fold(edition, String::removePrefix), String::removeSuffix)
        )
        .let {
          when {
            isSubstituteCard -> "${it.removeSuffix(" Placeholders")} Substitute Cards"
            isPromo && it == "Commander Legends: Battle for Baldur's Gate" ->
                "Battle for Baldur's Gate Promos"
            isToken && it == "Commander Legends: Battle for Baldur's Gate" ->
                "Battle for Baldur's Gate Tokens"
            isPromo &&
                it != IxalanTreasureChest &&
                it != STORE_CHAMPIONSHIPS &&
                it != GRN_RAVNICA_WEEKEND &&
                it != RNA_RAVNICA_WEEKEND &&
                !it.endsWith(" Standard Showdown") &&
                !it.startsWith("Friday Night Magic ") ->
                "${it.removeSuffix(" Store Championship")} Promos"
            isToken -> "$it Tokens"
            else -> it
          }
        }

fun editionToSetName(
    editionCode: String,
    edition: String,
    printingNote: String,
    deckboxCardName: String,
    isPromo: Boolean,
    isTheListCardStoredAsRegularVersionWithArtistProofTag: Boolean,
    knownSets: Map<String, String>,
): MetaInfo {
  val isSubstituteCard = deckboxCardName == "Double-Faced Card Placeholder"
  val printingNote =
      if (isTheListCardStoredAsRegularVersionWithArtistProofTag) editionCode else printingNote
  val editionCode =
      if (isTheListCardStoredAsRegularVersionWithArtistProofTag) "plist" else editionCode
  return when (val lowercaseEditionCode = editionCode.lowercase()) {
    "plist" -> {
      val isToken = false
      val setCode =
          mapEditionCode(printingNote, isSubstituteCard).takeIf { it in knownSets }
              ?: throw Exception("The List Card set code $printingNote unknown")
      ...
      MetaInfo(
          ...
          true,
          isSubstituteCard,
          isSetNumberReliable = false,
      )
    }
    "ptc" if deckboxCardName == "Silent Sentinel" ->
        MetaInfo("pbng", "Born of the Gods Promos", true, false, false, false)
    "rep" if deckboxCardName == "Rukh Egg" ->
        MetaInfo("p8ed", "Eight Edition Promos", true, false, false, false)
    "pm20" if deckboxCardName in listOf("Plains", "Island", "Swamp", "Mountain", "Forest") ->
        MetaInfo("ppp1", "M20 Promo Packs", true, false, false, false)
    "prw2" if deckboxCardName == "Lavinia, Azorius Renegade" ->
        MetaInfo("prna", "Ravnica Allegiance Promos", true, false, false, false)
    "gpx" ->
        MetaInfo("pf19", "MagicFest 2019", true, false, false, false, isSetNumberReliable = false)
    "ssp_1" if deckboxCardName == "Steel Leaf Champion" ->
        MetaInfo("pdom", "Dominaria Promos", true, false, false, false)
    "ssp_1" if deckboxCardName == "Ghalta, Primal Hunger" ->
        MetaInfo("prix", "Rivals of Ixalan Promos", true, false, false, false)
    "plgs" if deckboxCardName == "Reliquary Tower" || deckboxCardName == "Hangarback Walker" ->
        MetaInfo("plg20", "Love Your LGS 2020", true, false, false, false)
    "plgs" if deckboxCardName == "Thought Vessel" || deckboxCardName == "Sol Ring" ->
        MetaInfo("plg22", "Love Your LGS 2022", true, false, false, false)
    "fnmp" if deckboxCardName == "Grisly Salvage" ->
        MetaInfo("f13", "Friday Night Magic 2013", true, false, false, false)
    "fnmp" -> {
      val isPromo = true
      val isToken = false
      val regex =
      ...
      val setName =
          determineScryfallSetName(
              if (regex.find(printingNote) != null) {
      ...
      val setCode =
          setCodeBySetName(setName, knownSets)
              ?: throw Exception("Card from PromoSet with Actual Set ($setName) unknown")
    ...
    in promoSetCodeWithActualEditionInPrintingNode -> {
      ...
      val setCode =
          setCodeBySetName(setName, knownSets)
              ?: throw Exception(
                  "Card from PromoSet with Actual Set in PrintingNote ($setName) unknown"
              )
      ...
      val isPromo =
          isPromo ||
              promoPrefixes.any(edition::startsWith) ||
              promoSuffixes.any(edition::endsWith) ||
              printingNote in promoPrintingNotes ||
              lowercaseEditionCode in dedicatedPromoSets
      ...
      val setCode =
          mapEditionCode(lowercaseEditionCode, isSubstituteCard).takeIf { it in knownSets }
              ?: setCodeBySetName(setName, knownSets)
              ?: lowercaseEditionCode
...
fun Raise<Throwable>.getCard(
  ...
  return cardByNumber(setCode)
      ...
      ?: raise(Exception("card $cardNumber (\"$cardName\") not found in set [$setCode] $setName"))
}

fun mapEditionCode(editionCode: String, isSubstituteCard: Boolean = false): String =
    editionCode
        .lowercase()
        .let {
        ...
        .let { setCode -> setCodeMapping.getOrDefault(setCode, setCode) }
...
val promoSetCodeWithActualEditionInPrintingNode =
    listOf(
        "mlp",
        "lpromos",
        "bftcp",
...
/** key is deckbox name value is scryfall name */
val setNameMapping =
    mutableMapOf(
        ...
        "Fate Reforged Clash Pack Promos" to
            "Fate Reforged Clash Pack", // TODO eventually integrate CP2 cards as promo cards to FRF
        ...
        "The Lord of the Rings: Tales of Middle-earth Commander" to
        ...
        "Oversized: Adventures in the Forgotten Realms" to
            "Adventures in the Forgotten Realms Oversized",
    )

fun mapSetNumber(setNumber: String, setName: String) =
    setNumberMapping[setName]?.let { mapData ->
      setNumber
          .toIntOrNull()
          ?.let { setNumberInt ->
            val delta =
                mapData.asSequence().firstOrNull { it.key.contains(setNumberInt) }?.value ?: 0
            setNumberInt + delta
          }
          .toString()
    } ?: setNumber

val setNumberMapping =
    mutableMapOf<String, Map<IntRange, Int>>(
        "Portal Second Age" to
            mapOf(
                1..30 to +60,
                61..90 to +60,
                121..150 to -120,
...
fun String.parseLanguageDeckbox(): CardLanguage =
    when (this) {
