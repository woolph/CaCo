            ...
            it.use {
              try {
                transcoder.transcode(
                    TranscoderInput(ByteArrayInputStream(svgMod(byteArray))),
                    TranscoderOutput(it),
                )
              } catch (e: Exception) {
...
val mythicBinderLabelIconRenderer =
    IconRenderer.gradientModded("-mythic", "black", "#c54326", "#f7971c")
val rareBinderLabelIconRenderer =
    IconRenderer.gradientModded("-rare", "black", "#8d7431", "#f6db94")
val uncommonBinderLabelIconRenderer =
    IconRenderer.gradientModded("-uncommon", "black", "#626e77", "#c8e2f2")
...
suspend fun IconRenderer.cachedImage(
    ...
    ImageCache.getImageByteArray("$cacheId$cacheIdSuffix") {
      renderSvg(uri).onLeft { IconRenderer.log.error("couldn't get icon for $uri", it) }.getOrNull()
    }
...
fun lazyIcon(
    ...
    nullableUri?.let { uri -> lazy { runBlocking { iconRenderer.cachedImage(cacheId, uri) } } }
        ?: lazyOf(null)
