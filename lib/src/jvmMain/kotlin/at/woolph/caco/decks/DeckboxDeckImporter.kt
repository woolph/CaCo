...
class DeckboxDeckImporter(
  ...
  suspend fun importDeckboxDecks(userName: String) =
      getListOfDeckLinks(userName).map { link -> importDeck(link) }
  ...
  suspend fun getListOfDeckLinks(userName: String): Flow<URL> = flow {
    ...
    progress?.updateContext("fetching list of decks from $userUrl")

    val doc = withContext(Dispatchers.IO) { Jsoup.connect(userUrl).get() }

    val result = doc.select("li.deck")

    progress?.setTotal(result.size)

    emitAll(
        doc.select("li.deck")
            .asFlow()
            .mapNotNull { "$baseUrl${it.selectFirst("a[href]")?.attr("href")}" }
            .map(URI::create)
            .map(URI::toURL)
  ...
  suspend fun importDeck(deckUrl: URL): DeckList {
    progress?.updateContext("fetching deck from $deckUrl")
    val deckDoc = withContext(Dispatchers.IO) { Jsoup.connect(deckUrl.toString()).get() }

    val deckName =
        deckDoc
            .selectFirst("div.section_title")
            ?.selectFirst("span")!!
            .text()
            .replace(Regex("^\\d+\\s*"), "")
    progress?.updateContext("processing $deckName")
    val format =
        when (deckDoc.selectFirst("div.deck_info_widget span.variant")?.text()) {
    ...
    val commanders =
        sequenceOf(
                deckDoc
                    .selectFirst("div#commander_info span:matches(Commander:) ~ div > a")
                    ?.text(),
    ...
    val mainboard =
        deckDoc
            ...
            .filter { !commanders.contains(it.first) }
            .toMap()
    val sideboard =
        deckDoc.selectFirst("table.sideboard")!!.select("tr[id]").associate {
          val count = it.selectFirst("td.card_count")!!.text()
