...
class Card(id: EntityID<UUID>) : ICard, UUIDEntity(id), Comparable<Card>, CardRepresentation {
  ...
  var finishes: EnumSet<Finish> by
  ...
  var priceNormal: CurrencyValue? by
      Cards.price.transform({ it?.value }, { it?.let { CurrencyValue.eur(it) } })
  var priceFoil: CurrencyValue? by
      Cards.priceFoil.transform({ it?.value }, { it?.let { CurrencyValue.eur(it) } })
  val priceEtched: CurrencyValue? get() = priceFoil
  //by      Cards.priceEtched.transform({ it?.value }, { it?.let { CurrencyValue.eur(it) } })
  ...
  var colorIdentity by
      Cards.colorIdentity.transform(
          {
            it.colorIdentity.fold(0) { acc: Int, manaColor: ManaColor ->
              (acc or (1 shl manaColor.ordinal))
            }
          },
          {
            ColorIdentity(
                ManaColor.entries
                    .asSequence()
                    .filter { manaColor -> (it and (1 shl manaColor.ordinal)) != 0 }
                    .toEnumSet()
            )
          },
      )
