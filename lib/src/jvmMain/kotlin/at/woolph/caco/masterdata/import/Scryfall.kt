    ...
    jsonSerializer
        ...
        .collect { scryfallCard ->
          try {
              ...
              null -> {
                val oracleId =
                    scryfallCard.oracle_id
                        ?: scryfallCard.card_faces?.firstNotNullOf { it.oracle_id }
                        ?: throw IllegalStateException(
                            "no orcacle_id for scryfall_id ${scryfallCard.id}"
                        )
            ...
            if (
                t.setType != "memorabilia" || scryfallCard.set in ScryfallSet.memorabiliaWhiteList
            ) {
              log.e(t) { "error while importing card ${scryfallCard.name}" }
            } else {
              log.d(
                  "not importing card ${scryfallCard.name} cause set ${scryfallCard.set} is not to be imported"
              )
            }
          } catch (t: Throwable) {
            log.e(t) { "error while importing card ${scryfallCard.name}" }
      ...
      determineOriginalCardFor(scryfallCard, variantType)
          .onRight { originalCard ->
            try {
              ...
              log.e(
                  "error while importing variant card ${scryfallCard.id} ${scryfallCard.uri} ${scryfallCard.name} (which is considered to be a variant of type $variantType): ${e.message}"
              )
            }
          }
          .onLeft { t ->
...
internal fun getSetByCode(setCode: String) =
    Either.catch { ScryfallCardSet.find { ScryfallCardSets.code eq setCode }.single() }
      ...
      .collect { scryfallCard ->
        try {
          CardPrint.findByIdAndUpdate(scryfallCard.id) { cardPrint ->
            ...
            cardPrint.priceNormal =
                scryfallCard.prices["eur"]?.toDouble()?.let { CurrencyValue.eur(it) }
            cardPrint.priceFoil =
                scryfallCard.prices["eur_foil"]?.toDouble()?.let { CurrencyValue.eur(it) }
