...
class Frame(val parentNode: Node, box: PDRectangle) :
    Node(parentNode.document, parentNode.contentStream, box)

fun Node.frame(box: PDRectangle = this.box, block: Frame.() -> Unit) = Frame(this, box).apply(block)
...
fun Node.frameRelative(
    ...
    Frame(
            this,
            box.relative(horizontalAlignment, offsetX, verticalAlignment, offsetY, width, height),
        )
        .apply(block)
...
fun Page.framePagePosition(
    ...
    Frame(this, box.inset(pagePosition, marginInner, marginTop, marginOuter, marginBottom))
        .apply(block)
...
fun ByteArray.toPDImage(node: Node) =
    PDImageXObject.createFromByteArray(node.document.document, this, null)
...
fun <Item> PDFDocument.columnedContent(
    items: Iterable<Item>,
    pageFormat: PDRectangle,
    columns: Int,
    columnBlock: Frame.(Item) -> Unit,
) {
  ...
  items.chunked(columns).forEachIndexed { pageIndex, mapLabelItems ->
    page(pageFormat) {
      ...
      mapLabelItems.forEachIndexed { i, set ->
        frame(columnWidth * i, 0f, (columnWidth) * (columns - i - 1), 0f) { columnBlock(set) }
      }
    }
  }
}
