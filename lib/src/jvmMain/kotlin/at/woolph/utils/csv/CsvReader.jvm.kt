...
actual class CsvReader actual constructor(file: Path, explicitHeader: Array<String>?) :
  ...
  private val csvReader =
      CSVReader(SystemFileSystem.source(file).buffered().asInputStream().bufferedReader())
  actual val header: Map<String, Int>

  init {
    header = (explicitHeader ?: csvReader.readNext()).withIndex().associate { it.value to it.index }
  }
