...
open class CardPossessionModel(card: Card, val collectionsSettings: CollectionSettings) :
    CardModel(card) {

  val possessionPremiumTarget: IntegerBinding =
      integerBinding(extendedArt, nonfoilAvailable, foilAvailable) {
  ...
  val possessionNonPremiumTarget: IntegerBinding =
      integerBinding(
          extendedArt,
          specialDeckRestrictions,
          nonfoilAvailable,
          possessionPremiumTarget,
      ) {
        if (extendedArt.value || !nonfoilAvailable.value) {
          0
        } else {
          specialDeckRestrictions.value?.let {
            if (it > 0) min(it, collectionsSettings.cardPossessionTargetNonPremiumNoDeckRestriction)
            else null
          } ?: collectionsSettings.cardPossesionTargtNonPremium
        }
      }
  ...
  val collectionCompletion =
      stringBinding(
          possessionNonPremium,
          possessionPremium,
          possessionNonPremiumTarget,
          possessionPremiumTarget,
      ) {
        ...
        val plus =
            if (possessionNonPremium.get() > possessionNonPremiumTarget.value)
                CollectionView.ICON_REDUNDANT_OWNED_CARD
            else " "
        val plusFoil =
            if (possessionPremium.get() > possessionPremiumTarget.value)
                CollectionView.ICON_REDUNDANT_OWNED_CARD
            else " "
  ...
  fun getPaperPossessionsMap(
      languages: Set<CardLanguage>,
      conditions: Set<CardCondition>,
  ): Map<CardLanguage, Map<CardCondition, Possession>> {
        ...
        val count =
            possessions.count {
              it.language == language &&
                  (condition == null || it.condition == condition) &&
                  it.finish == Finish.Normal
            }
        val foilCount =
            possessions.count {
              it.language == language &&
