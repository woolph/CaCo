...
suspend fun <T> Dialog<T>.showAndAwait(): T? =
    withContext(Dispatchers.Main.immediate) { this@showAndAwait.showAndWait().orElse(null) }

fun ButtonBase.action(coroutineScope: CoroutineScope, op: suspend () -> Unit) = setOnAction {
  coroutineScope.launch(Dispatchers.Main.immediate) { op() }
}

val emptyImage = Image(CollectionView::class.java.getResourceAsStream("/empty.png"))

fun imageViewDelayed(
    ...
    ImageView(emptyImage).apply {
      ...
      coroutineScope.launch(Dispatchers.IO) {
        block()?.let { withContext(Dispatchers.Main) { this@apply.image = it } }
      }
    }

abstract class CoroutineScopedView
@JvmOverloads
constructor(title: String? = null, icon: Node? = null) : View(title, icon) {
  ...
  init {
    whenUndocked {
      coroutineScope.coroutineContext.cancelChildren(
          CancellationException("undocking $coroutineScopedViewName")
      )
    }
    whenDocked { jobs.forEach { coroutineScope.launch(Dispatchers.Main.immediate) { it(this) } } }
  }
}
...
abstract class CollectionView(val collectionSettings: CollectionSettings) : CoroutineScopedView() {
  ...
  val setsFiltered =
      setsSorted.filteredBy(listOf(setFilterTextProperty)) { set ->
      ...
      cardsSorted.filteredBy(
        ...
        cardInfo.filterView() &&
            (filterTextProperty.get().isBlank() ||
                cardInfo.names.any { it.contains(filterTextProperty.get(), ignoreCase = true) }) &&
  ...
  suspend fun updateSetsView(updatedSets: List<CardSetModel>) =
  ...
  suspend fun updateCards() {
    newSuspendedTransaction(Dispatchers.IO) {
      ...
      val updatedCards =
          set?.item?.cardsOfSelfAndNonRootChildSets?.toList()?.sorted()?.map {
            CardPossessionModel(it, collectionSettings)
          } ?: emptyList()
  ...
  suspend fun updateCardsView(updatedCards: List<CardPossessionModel>) =
      withContext(Dispatchers.Main.immediate) {
  ...
  override val jobs: List<suspend (CoroutineScope) -> Unit> =
      listOf(
          { initSets() },
    ...
    with(root) {
      ...
      top {
        toolbar {
          ...
          button("\u21BB All") {
            action(coroutineScope) { newSuspendedTransaction { importAllSets() } }
          }
          button("\u21BB") {
            action(coroutineScope) { newSuspendedTransaction { set?.item?.reimportSet() } }
          }
          button("+") {
            action(coroutineScope) {
              AddSetsDialog(this@CollectionView).showAndAwait()?.let { setCodes ->
                newSuspendedTransaction {
                  ...
                  setListview.selectionModel.select(
                      setCodes
                          .split(',')
                          .map { addSet(it.trim().lowercase(Locale.getDefault())) }
                          .last()
                          .let(::CardSetModel)
                  )
                }
          ...
          togglebutton(CARD_IN_POSSESION) { filterCompleteProperty.bind(selectedProperty()) }
          if (collectionSettings.cardPossesionTargtNonPremium > 0) {
            togglebutton(
                NONFOIL_IN_POSSESION.repeat(collectionSettings.cardPossesionTargtNonPremium)
            ) {
          ...
          togglebutton("C") { filterRarityCommon.bind(selectedProperty()) }
          togglebutton("U") { filterRarityUncommon.bind(selectedProperty()) }
          togglebutton("R") { filterRarityRare.bind(selectedProperty()) }
          togglebutton("M") { filterRarityMythic.bind(selectedProperty()) }
          region {
            prefWidth = 40.0
      ...
      center {
        splitpane {
                ...
                setRowFactory {
                  object : TableRow<CardPossessionModel>() {
                    override fun updateItem(cardInfo: CardPossessionModel?, empty: Boolean) {
                      ...
                      tooltip =
                          cardInfo?.let {
                            CardImageTooltip(it, toggleButtonImageLoading.selectedProperty())
                          }
                    }
                  }
                }
                  ...
                  if (toggleButtonImageLoading.isSelected) {
                    coroutineScope.launch(Dispatchers.Default) {
                      flowOf(+1, -1, +2, +3)
                          ...
                          .collect { coroutineScope.launch(Dispatchers.IO) { it.cacheImage() } }
                    }
                  }
                }
              }
          ...
          vbox {
            ...
            this +=
                find<CardPossessionView> {
                  runLater { this.cardProperty.bind(tvCards.selectionModel.selectedItemProperty()) }
                }
          }
        }
      }
