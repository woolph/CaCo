...
class DecksView : View() {
  ...
  fun updateArchetypes() {
    archetypes.setAll(transaction { DeckArchetype.all().map { DeckArchetypeModel(it) } })
  }

  fun updateDecks() {
    decks.setAll(transaction { Build.all().map { DeckBuildModel(it) } })
  }
    ...
    with(root) {
      ...
      left {
        splitpane(orientation = Orientation.VERTICAL) {
                ...
                setRowFactory {
                  object : TableRow<DeckTreeModel>() {
                    override fun updateItem(deckModel: DeckTreeModel?, empty: Boolean) {
                      ...
                      tooltip =
                          deckModel?.comment?.value?.let {
                            if (it.isNotBlank()) Tooltip(it) else null
                          }
          ...
          tvDecks =
              tableview(decksFiltered) {
                hboxConstraints { hGrow = Priority.ALWAYS }
                vboxConstraints { vGrow = Priority.ALWAYS }
                column("Archetype", DeckBuildModel::archetypeName) { remainingWidth() }
                column("Name", DeckBuildModel::name) { remainingWidth() }
  ...
  fun addNewDeck(initialArchetype: DeckArchetypeModel? = null) {
    transaction {
      AddDeckBuildDialog(this@DecksView, initialArchetype?.item).showAndWait().ifPresent {
          (archetype, subname, comment) ->
        Build.new {
  ...
  private fun getContextMenu(deck: DeckTreeModel?) =
      if (deck != null)
          ContextMenu().apply {
            ...
            checkmenuitem("Archive") { selectedProperty().bindBidirectional(deck.archived) }
          }
      else ContextMenu().apply { item("Add new archetype") { action { addNewArchetype() } } }

  private fun getContextMenu(deck: DeckArchetypeModel?) =
      if (deck != null)
          ContextMenu().apply {
            item("Delete") {
              action {
                confirmation(
                    "Are you sure you want to delete archetype $deck?",
                    null,
                    ButtonType.YES,
                    ButtonType.NO,
                ) {
  ...
  private fun getContextMenu(deck: DeckBuildModel?) =
      if (deck != null)
          ContextMenu().apply {
            item("Delete") {
              action {
      ...
      else ContextMenu().apply { item("Add new archetype") { action { addNewDeck() } } }
}
