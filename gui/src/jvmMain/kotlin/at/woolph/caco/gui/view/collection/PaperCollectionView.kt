    ...
    button("Print Inventory") {
      ...
      action(coroutineScope) {
        chooseFile(
          ...
          // TODO progress dialog
          val fontTitle = Font(PDType1Font(Standard14Fonts.FontName.HELVETICA_BOLD)).withSize(10f)
          ...
          .setContents(
            StringSelection(transaction {
              cardsSorted.asSequence().map { card ->
                val neededCount =
                  kotlin.math.max(0, card.possessionNonPremiumTarget.value - card.possessionNonPremium.value)
