...
class SetDetailsView(
    val setProperty: SimpleObjectProperty<CardSetModel?> = SimpleObjectProperty(null)
) : Fragment() {
  ...
  init {
    labelNumberInSet
        .textProperty()
        .bind(setProperty.selectNullable { it?.officalCardCount }.toStringBinding())
    labelSetName.textProperty().bind(setProperty.selectNullable { it?.name })

    coroutineScope.launch(Dispatchers.Main.immediate) {
      setProperty.asFlow().collectLatest { set ->
