...
class EnglishPaperCollectionView : CollectionView(COLLECTION_SETTINGS) {
        ...
        CollectionSettings(
            ...
            { !it.digitalOnly },
            {
              it.possessions.count {
                it.language == CardLanguage.ENGLISH && it.finish == Finish.Normal
              }
            },
            {
              it.possessions.count {
  ...
  override fun ToolBar.addFeatureButtons() {
    ...
    button("Export Missing") {
      action {
        chooseFile(
                "Choose File to Wants",
                arrayOf(FileChooser.ExtensionFilter("Text", "*.txt")),
                mode = FileChooserMode.Save,
            )
            .singleOrNull()
            ?.let {
                  ...
                  set?.item
                      ...
                      ?.forEach {
                        val neededCount =
                            max(
                                0,
                                collectionSettings.cardPossesionTargtNonPremium -
                                    it.possessions.count(),
                            )
                        // val n = if(set.cards.count { that -> it.name == that.name } > 1) "
                        // (#${it.numberInSet})" else ""
                        if (neededCount > 0) {
