...
class CardPossessionView(
    val cardProperty: SimpleObjectProperty<CardPossessionModel?> = SimpleObjectProperty(null)
) : Fragment() {
  ...
  private val possessionMap =
      cardProperty.mapBinding { it?.getPaperPossessionsMap(languages, conditions) ?: emptyMap() }
  private val possessions =
      languages.associateWith { language ->
        conditions.associateWith { condition ->
          possessionMap.mapStringBinding {
            it?.get(language)?.get(condition)?.let(CardPossessionModel.Possession::asString) ?: ""
          }
        }
      }
      ...
      languages.associateWith { language ->
        possessionMap.mapStringBinding {
          it?.get(language)
              ?.values
              ?.fold(
                  CardPossessionModel.Possession(0, 0),
                  CardPossessionModel.Possession.Companion::fold,
              )
  ...
  private fun CardLanguage.toIcon() =
      when (this) {
  ...
  private fun CardCondition.toIcon() =
      when (this) {
  ...
  override val root = gridpane {
    ...
    row {
      label("")
      languages.forEach { cl -> add(cl.toIcon()) }
    }
    row {
      label("\u2211") { addClass(Styles.sumRow) }
    ...
    conditions.forEach { cc ->
      row {
        ...
        languages.forEach { cl -> label { textProperty().bind(possessionsStringBinding(cl, cc)) } }
      }
    }
