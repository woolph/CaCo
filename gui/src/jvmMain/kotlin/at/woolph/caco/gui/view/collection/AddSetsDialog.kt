...
class DialogButtonsBuilder<R>(val dialog: Dialog<R>) {
  protected var defaultConverter: () -> R = {
    throw IllegalStateException("no default action defined")
  }
  ...
  inner class DialogButtonBuilder(val buttonType: ButtonType) {
    ...
    internal fun doAction() = converter?.let { it(buttonType) } ?: defaultConverter()
  }
...
class AddSetsDialog(val owner: View) : Dialog<String?>() {
  ...
  init {
    ...
    buttons {
      button(ButtonType.APPLY) { action { setCode.lowercase(Locale.getDefault()) } }
