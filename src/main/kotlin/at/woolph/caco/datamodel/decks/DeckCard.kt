...
object DeckCards : IntIdTable() {
    ...
    val roles = long("roles").index().default(0L).transform(
        { roles: EnumSet<CardRole> -> roles.fold(0L) { acc, cr2 -> acc or (1L shl cr2.ordinal) } },
        { roles: Long -> CardRole.values().filter { ((1L shl it.ordinal) and roles) != 0L }.fold(EnumSet.noneOf(CardRole::class.java)) { acc, cardRole ->
                acc.add(cardRole)
                acc
            }
        }
