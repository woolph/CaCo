...
class SetWithCommanderAndMasterPieces(override val code: String, subCode1: String, subCode2: String): MapLabelItem {
	...
	init {
		val (mainSet, commanderSet, masterpieceSet) = transaction {
			arrayOf(
				CardSet.findById(code) ?: throw IllegalArgumentException("no set with code $code found"),
				CardSet.findById(subCode1) ?: throw IllegalArgumentException("no set with code $subCode1 found"),
				CardSet.findById(subCode2) ?: throw IllegalArgumentException("no set with code $subCode2 found"),
			)
		}
...
class FiveSetBlock(blockTitle: String, code0: String, code1: String, code2: String, code3: String, code4: String): MapLabelItem {
	...
	init {
		...
		val cardSets = transaction {
			codes.map { _code ->
				CardSet.findById(_code) ?: throw IllegalArgumentException("no set with code $_code found")
			}
		}
...
//				SimpleSet("7ed"),
//				SimpleSet("2x2"),
//				SetWithCommander("dmu", "dmc"),
//				object: SimpleSet("clb") {
//					override val subCode: String = "PRECONS"
//					override val subTitle = "Preconstructed Decks"
//				},
//				SimpleSet("40k"),
...
//				ThreeSetBlock("Ice Age Block", "ice", "all", "csp"),
			...
			mapLabels.filter { it.value.isNotEmpty() }.forEach { (columns, labels) ->
