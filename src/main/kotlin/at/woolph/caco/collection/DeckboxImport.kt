...
fun Raise<Throwable>.mapDeckbox(nextLine: CsvRecord, knownSets: Map<String, String>): CardCollectionItem {
    ...
    if (token && cardName.contains(" // ")) {
        raise(IntentionallySkippedException("skipping $cardName because it is an Double Sided Token which is not supported"))
        ...
        raise(IntentionallySkippedException("skipping $cardName cause Ability Punchcard's are not covered by scryfall"))
    } else {
        val card = getCard(
            setCode,
            setName,
        ...
        return CardCollectionItem(
            ...
            cardCollectionItemId = CardCollectionItemId(
                ...
                condition = condition,
                variantType = when {
                    stampPlaneswalkerSymbol && stampPrereleaseDate -> raise(IllegalStateException("Card can't be promopack stamped and prerelease stamped at the same time!"))
...
fun editionToSetName(editionCode: String, edition: String, printingNote: String, deckboxCardName: String, isPromo: Boolean, isTheListCardStoredAsRegularVersionWithArtistProofTag: Boolean, knownSets: Map<String, String>): MetaInfo {
    ...
    return when(val lowercaseEditionCode = editionCode.lowercase()) {
        "plist" -> {
            val isToken = false
            val setCode = mapEditionCode(printingNote, isSubstituteCard).takeIf { it in knownSets }
...
fun Raise<Throwable>.getCard(setCode: String, setName: String, cardNumber: String, token: Boolean, cardName: String, rarity: Rarity?, manaCost: String?, promo: Boolean): Card {
