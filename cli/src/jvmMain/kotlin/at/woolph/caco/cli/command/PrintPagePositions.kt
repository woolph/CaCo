...
class PrintPagePositions : SuspendingCliktCommand(name = "page-position") {
  ...
  val loadingBackside by
      option(help = "Flag whether backside is loadable too").boolean().default(true)

  override suspend fun run() {
    val pagePositionCalculator =
        PagePositionCalculator(
            PagePositionCalculator.PageMetadata("", rowsPerPage, columnsPerPage, loadingBackside)
        )

    generateSequence { terminal.prompt("Set number") { ConversionResult.Valid(it.toIntOrNull()) } }
