...
class PrintMissing : SuspendingTransactionCliktCommand(name = "missing") {
  ...
  override suspend fun runTransaction() {
    val set =
        ScryfallCardSet.findByCode(setCode)
            ?: throw IllegalArgumentException("Set $setCode not found")
    set.cardPrints
        .filter { cardPrint ->
        ...
        .forEach { (type, cards) ->
          println("$type:")
          cards.sorted().forEach { card -> println("1x ${card.mergedName} (${card.set.name})") }
        }
  }
